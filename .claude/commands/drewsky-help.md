...
### `/tcrei`
**Apply TCREI Validation** - Structure your request with Task, Context, Reference, Evaluation, Input validation. Eliminates ambiguity and improves output quality.

*Use when:* Starting any task, clarifying requirements, reducing back-and-forth

### `/maker`
**Apply MAKER Decomposition** - Break complex tasks into Manageable, Atomic, Knowable, Executable, Reviewable steps. Prevents overwhelm and tracks progress.

*Use when:* Facing complex multi-step tasks, need clear roadmap, want to prevent scope creep

### `/verify`
**Apply Chain of Verification (CoVe)** - Meta AI's 4-step fact-checking process. Reduces hallucinations by 23% through systematic verification.

*Use when:* Answering questions about code, making recommendations, uncertain about claims

### `/plan`
**Apply Dual-Loop Planning** - Microsoft's Magentic-One strategic + tactical planning. Separates high-level vision from concrete execution.

*Use when:* Large refactors, migrations, architectural changes, anything taking >30min
...
**Simple bug fix?** → Use `/tcrei` to clarify scope
...
- **Meta AI**: Chain of Verification (23% hallucination reduction)
- **Microsoft**: Magentic-One dual-loop planning (stall detection)
- **Stanford/SambaNova**: Reflective learning (skill extraction)
